...
/** The active banner for one marketplace (or the main homepage, when $marketplaceTypeId is null). */
function mp_active_banner(?int $marketplaceTypeId): ?array
{
    if ($marketplaceTypeId === null) {
        return mp_db_fetch_one(
            'SELECT * FROM cms_banners WHERE marketplace_type_id IS NULL AND is_active = 1 ORDER BY sort_order ASC LIMIT 1'
        );
    }

    return mp_db_fetch_one(
        'SELECT * FROM cms_banners WHERE marketplace_type_id = ? AND is_active = 1 ORDER BY sort_order ASC LIMIT 1',
        [$marketplaceTypeId]
    );
}
...
{
    return mp_db_fetch_one('SELECT * FROM cms_banners WHERE id = ? LIMIT 1', [$id]);
}
