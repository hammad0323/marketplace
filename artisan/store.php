...
<section class="artisan-story reveal">
    ...
    <form method="post" action="<?= mp_e(ROUTE_STORE) ?>follow.php?vendor_id=<?= (int) $vendor['id'] ?>" class="inline-form">
        <?= mp_csrf_field() ?>
        <input type="hidden" name="redirect_to" value="store.php?slug=<?= mp_e($vendor['slug']) ?>">
