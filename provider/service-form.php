...
// A provider's category is fixed at registration (see provider/register.php)
// and every service/product they list belongs to it — there's no per-service
// category picker any more.
$categoryId = $service ? (int) $service['category_id'] : (int) $provider['category_id'];
$category = $categoryId ? db_select_one($conn, 'SELECT * FROM categories WHERE id = ? AND is_active = 1', [$categoryId]) : null;

if (!$category) {
    flash_set('danger', 'Your account has no valid category set — contact support.');
    redirect('/provider/services.php');
}

$isProduct = $category['listing_type'] === 'product';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $old['stock_quantity'] = isset($_POST['stock_quantity']) && $_POST['stock_quantity'] !== '' ? (int) $_POST['stock_quantity'] : '';
    if ($isProduct) {
        $old['price_unit'] = 'fixed';
    }
    ...
    if ($isProduct && $old['stock_quantity'] === '') {
    ...
    if (!$errors) {
        ...
        $params = [
            ...
            $old['max_guests'] ?: null, $isProduct ? $old['stock_quantity'] : null, $old['cancellation_policy'],
        ];

        if ($service) {
            db_execute(
                $conn,
                'UPDATE services SET category_id=?, city_id=?, title=?, slug=?, short_description=?, description=?, address=?, latitude=?, longitude=?, price=?, price_unit=?, max_guests=?, stock_quantity=?, cancellation_policy=?, status="pending" WHERE id=?',
                array_merge($params, [(int) $service['id']])
            );
            $serviceId = (int) $service['id'];
            flash_set('success', ($isProduct ? 'Product' : 'Service') . ' updated and resubmitted for approval.');
        } else {
            $serviceId = db_insert_get_id(
                $conn,
                'INSERT INTO services (provider_id, category_id, city_id, title, slug, short_description, description, address, latitude, longitude, price, price_unit, max_guests, stock_quantity, cancellation_policy, status)
                 VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?, "pending")',
                array_merge([(int) $provider['id']], $params)
            );
            flash_set('success', ($isProduct ? 'Product' : 'Service') . ' submitted for approval.');
        }
...
$pageTitle = $service ? "Edit $itemWord" : "New $itemWord";
require ROOT_PATH . '/includes/header.php';
?>
<div class="section-tight">
  <div class="container-xl" style="max-width:820px;">
    <a href="<?php echo url('/provider/services.php'); ?>" style="color:var(--ink-mute);font-size:13.5px;"><i class="bi bi-arrow-left"></i> Back to <?php echo $isProduct ? 'products' : 'services'; ?></a>

    <div class="section-head" style="margin-top:16px;">
      <span class="eyebrow"><i class="bi <?php echo e($category['icon'] ?: 'bi-tag'); ?>"></i> <?php echo e($category['name']); ?></span>
      <h1 class="section-heading"><?php echo $service ? "Edit $itemWord" : "New $itemWord"; ?></h1>
      <p class="section-sub">Listed under <?php echo e($category['name']); ?> — that's fixed to your business type, so there's nothing to choose here.</p>
    </div>
    ...
    <form method="post" class="form-w panel" enctype="multipart/form-data">
      ...
      <?php if ($isProduct): ?>
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:0 20px;">
          <div><label>Price (per item)</label><input type="number" step="0.01" min="0.01" name="price" value="<?php echo e($old['price']); ?>" required></div>
          <div><label>Stock quantity</label><input type="number" min="0" step="1" name="stock_quantity" value="<?php echo e($old['stock_quantity']); ?>" required></div>
        </div>
        <input type="hidden" name="price_unit" value="fixed">
      <?php else: ?>
      ...
      <label style="margin-top:20px;"><?php echo $isProduct ? 'Return & refund policy' : 'Cancellation policy'; ?></label>
