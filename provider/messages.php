...
$conversation = $conversationId ? db_select_one(
    $conn,
    'SELECT c.*, u.name AS customer_name FROM conversations c JOIN users u ON u.id = c.customer_id WHERE c.id = ? AND c.provider_id = ?',
    [$conversationId, (int) $provider['id']]
) : null;
...
<div class="section-tight">
  <div class="container-xl">
    <div class="section-head">
      <span class="eyebrow"><i class="bi bi-chat-dots"></i> Provider</span>
          ...
          <?php foreach ($conversations as $c): ?>
            <a href="?conversation_id=<?php echo (int) $c['id']; ?>" style="display:block;padding:14px 16px;border-bottom:1px solid var(--border);<?php echo $conversationId === (int) $c['id'] ? 'background:var(--purple-50);' : ''; ?>">
              <div style="display:flex;justify-content:space-between;">
                <strong style="font-size:14px;"><?php echo e($c['customer_name']); ?></strong>
              ...
              <div style="font-size:12.5px;color:var(--ink-mute);margin-top:3px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;"><?php echo e($c['last_message'] ?? ''); ?></div>
            </a>
          <?php endforeach; ?>
        <?php else: ?>
          <div class="empty-state" style="padding:32px;"><div class="icon-wrap"><i class="bi bi-chat-dots"></i></div><h4>No conversations yet</h4></div>
        <?php endif; ?>
      </div>

      <div class="panel" style="display:flex;flex-direction:column;padding:0;">
        <?php if ($conversation): ?>
          <div style="padding:14px 18px;border-bottom:1px solid var(--border);font-weight:700;"><?php echo e($conversation['customer_name']); ?></div>
