...
<div class="tp-container py-5">
  ...
  <div class="d-flex gap-2 flex-wrap mb-4">
    <a href="<?= tp_url('blog') ?>" class="btn btn-sm <?= !$category ? 'btn-dark' : 'btn-outline-secondary' ?>">All</a>
    <?php foreach ($blogCategories as $bc): ?>
      <a href="<?= tp_url('blog?category=' . $bc['slug']) ?>" class="btn btn-sm <?= ($category['id'] ?? null) === $bc['id'] ? 'btn-dark' : 'btn-outline-secondary' ?>"><?= e($bc['name']) ?></a>
    <?php endforeach; ?>
  </div>
  ...
  <div class="row g-4">
    <?php foreach ($posts as $post): ?>
      <div class="col-md-4">
        <a href="<?= tp_url('blog/' . $post['slug']) ?>" class="tp-card text-decoration-none d-block h-100 overflow-hidden">
