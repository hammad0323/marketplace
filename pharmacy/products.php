...
$stmt = mysqli_prepare(db(), 'SELECT verification_status FROM pharmacies WHERE id = ? LIMIT 1');
mysqli_stmt_bind_param($stmt, 'i', $pharmacyId);
mysqli_stmt_execute($stmt);
$verificationStatus = mysqli_stmt_get_result($stmt)->fetch_assoc()['verification_status'] ?? 'pending';
...
if ($isVerified) {
    $stmt = mysqli_prepare(db(), "SELECT p.*, c.name AS category_name FROM doctor_products p LEFT JOIN product_categories c ON c.id = p.category_id WHERE p.pharmacy_id = ? AND p.seller_type = 'pharmacy' ORDER BY p.created_at DESC");
    mysqli_stmt_bind_param($stmt, 'i', $pharmacyId);
    mysqli_stmt_execute($stmt);
    ...
    $stmt = mysqli_prepare(db(), "
        ...
        WHERE o.pharmacy_id = ? AND o.seller_type = 'pharmacy'
    ...
    $extraScripts = '<script>window.PRODUCT_SAVE_URL="/ajax/pharmacy-product-save.php";window.PRODUCT_DELETE_URL="/ajax/pharmacy-product-delete.php";window.ORDER_UPDATE_URL="/ajax/pharmacy-order-update.php";</script><script defer src="' . asset_url('/assets/js/doctor-products.js') . '"></script>';
}
...
<div class="card" style="padding:48px 32px;text-align:center;max-width:560px;margin:0 auto;" data-reveal>
    ...
    <h3 style="margin-bottom:10px;">Verification required</h3>
...
<div class="doc-tab-panel" id="panel-catalog" role="tabpanel" aria-labelledby="tab-btn-catalog" tabindex="0">
    ...
    <?php if (!$products): ?>
    <div class="card empty-state" data-reveal><i class="ri-store-2-line"></i><h4>No listings yet</h4><p>Add a medicine or product to start selling from your public store page.</p></div>
    <?php else: ?>
    ...
    <div class="modal-box" style="grid-template-columns:1fr;max-width:560px;">
        ...
        <div style="padding:36px;">
            ...
            <form id="product-form">
                ...
                <label class="checkbox-row" style="margin-bottom:20px;"><input type="checkbox" name="is_active" value="1" checked> Active (visible on my public store page)</label>
