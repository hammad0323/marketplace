...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    csrf_verify();
    $home = max(1, min(4, (int)($_POST['active_home'] ?? 1)));
...
$homes = [
    1 => ['name' => 'Home 1', 'desc' => 'Inspired by Al Imran Fabrics — category grid, new arrivals, editorial split, best sellers.'],
    2 => ['name' => 'Home 2', 'desc' => 'Inspired by Gul Ahmed — trending slider, promo banners, brand story, testimonials.'],
    3 => ['name' => 'Home 3', 'desc' => 'Inspired by Alkaram Studio — minimal full-width hero, featured edit, editorial split, Instagram feed.'],
    4 => ['name' => 'Home 4', 'desc' => 'Inspired by Zeen Woman — category tiles, festive edit, full-width promo, best sellers.'],
];
?>
<h1 class="page-title mb-2">Homepage Selector</h1>
<p class="text-muted mb-4">Choose which homepage design is live on the website. Each homepage has its own banners and sections you can manage independently.</p>

<form method="post">
<?= csrf_field() ?>
<div class="row g-4">
  <?php foreach ($homes as $num => $h): ?>
  <div class="col-md-6 col-lg-3">
    <label class="admin-card d-block h-100" style="cursor:pointer;<?= $active == $num ? 'border-color:var(--admin-primary);box-shadow:0 0 0 2px var(--admin-primary)' : '' ?>">
      ...
      <h2 class="h6"><?= e($h['name']) ?> <?= $active == $num ? '<span class="badge text-bg-success">Live</span>' : '' ?></h2>
      <p class="small text-muted mb-2"><?= e($h['desc']) ?></p>
