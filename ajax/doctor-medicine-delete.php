...
$stmt = mysqli_prepare($db, 'SELECT name, featured_image FROM medicine_info WHERE id = ? AND author_id = ? LIMIT 1');
...
$stmt = mysqli_prepare($db, 'DELETE FROM medicine_info WHERE id = ? AND author_id = ?');
mysqli_stmt_bind_param($stmt, 'ii', $id, $authorId);
mysqli_stmt_execute($stmt);
mysqli_stmt_close($stmt);
...
log_activity($authorId, 'doctor', 'delete_medicine_info', "Deleted medicine info: {$medicine['name']}");
