...
if ($op === 'remove') {
    $id = (int) ($_POST['id'] ?? 0);
    $stmt = mysqli_prepare($db, 'SELECT file_path FROM pharmacy_certificates WHERE id = ? AND pharmacy_id = ? LIMIT 1');
    mysqli_stmt_bind_param($stmt, 'ii', $id, $pharmacyId);
    mysqli_stmt_execute($stmt);
    $cert = mysqli_stmt_get_result($stmt)->fetch_assoc();
    mysqli_stmt_close($stmt);
    if ($cert) {
        $stmt = mysqli_prepare($db, 'DELETE FROM pharmacy_certificates WHERE id = ? AND pharmacy_id = ?');
        mysqli_stmt_bind_param($stmt, 'ii', $id, $pharmacyId);
        mysqli_stmt_execute($stmt);
        mysqli_stmt_close($stmt);
...
$stmt = mysqli_prepare($db, 'INSERT INTO pharmacy_certificates (pharmacy_id, title, issued_by, issued_year, file_path) VALUES (?, ?, ?, ?, ?)');
mysqli_stmt_bind_param($stmt, 'issis', $pharmacyId, $title, $issuedBy, $issuedYear, $result);
mysqli_stmt_execute($stmt);
